# 357期 CSS Mask Animations, Three.js Interiors, and a Direction Aware Glow

[CSS Mask Animations, Three.js Interiors, and a Direction Aware Glow](https://codepen.io/spark/357)

## Sunday CSS #10: 3D in CSS is not real

- [YouTube视频](https://www.youtube.com/watch?v=doKbDTWsMtw)
- [参考代码](https://codepen.io/miocene/pen/rNoyLyO)

实践学习
https://shuyang532.github.io/demo/3D_in_CSS_is_not_real.html

<iframe height="600" width="100%" scrolling="no" title="Zdog trefoil" src="[https://codepen.io/desandro/embed/XWNWPve?height=600&theme-id=light&default-tab=js,result](https://shuyang532.github.io/demo/3D_in_CSS_is_not_real.html)" frameborder="no" loading="lazy" allowtransparency="true" allowfullscreen="true">
  See the Pen <a href='https://codepen.io/desandro/pen/XWNWPve'>Zdog trefoil</a> by Dave DeSandro
  (<a href='https://codepen.io/desandro'>@desandro</a>) on <a href='https://codepen.io'>CodePen</a>.
</iframe>

[实践](https://shuyang532.github.io/demo/3D_in_CSS_is_not_real.html ':include')

## Recipe Card

- glassmorphic design
- [参考代码](https://codepen.io/hluebbering/pen/dywzPre)

实践学习

## CodePenChallenge: Navigation Bars

- [参考代码](https://codepen.io/collection/VYZrjQ)

实践学习

## Scroll Timeline

- 滚动阴影和滚动指向器
- [参考代码](https://codepen.io/shuding/pen/WNLGQor)

## Pure CSS Cat Responsive Animation | CSS Art

- [参考代码](https://codepen.io/Jopzik/pen/yLGMzOK)

## Glowing Hover Effect

- 发光效果
- [参考代码](https://codepen.io/finnchillah/pen/bGOqrXg)

## Revealing Images With CSS Mask Animations

- 蒙版动画显示图片
- [参考代码](https://www.smashingmagazine.com/2023/09/revealing-images-css-mask-animations/)


## 70s Ovals

- 复古椭圆样式
- [参考代码](https://codepen.io/loficodes/pen/qBLjaey)


## Wall Poster Animation

- 墙海报动画
- [参考代码](https://codepen.io/wakana-k/pen/MWZbVxm)
